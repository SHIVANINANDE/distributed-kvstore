...
func NewGRPCServer(cfg *config.Config, storageEngine storage.StorageEngine, logger *logrus.Logger) *GRPCServer {
	return &GRPCServer{
		config:  cfg,
...
func (s *GRPCServer) loggingInterceptor(
	...
	if err != nil {
		...
		logEntry.Debug("gRPC request completed")
	}
...
func (s *GRPCServer) Put(ctx context.Context, req *kvstore.PutRequest) (*kvstore.PutResponse, error) {
	...
	if err != nil {
		s.logger.WithError(err).WithField("key", req.Key).Error("Failed to put key")
		return &kvstore.PutResponse{
			Success: false,
			Error:   err.Error(),
